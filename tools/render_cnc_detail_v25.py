...
scene('01-exploded','Exploded structural cabinet / 36 wood assembly records',wood,'Exploded translations are for visibility only. Three 2t assemblies require separate ply IDs before nesting.',True)
...
scene('07-playfield','Cradle laps, bearing and independent safety load paths',[n for n in wood if n.startswith(('Cradle','SafetyStay','ClosedSupport','GasStrut'))]+[n for n in parts if n.startswith(('UCFL202','SafetyStayOpen','BearingBacking','PivotJournal','PivotPlate','ClosedSupportSeat'))],'15 mm journals; two independent positive stays; closed pads/latches. Strut mounts blocked until mass / CG exists.')
...
page=['<!doctype html><meta charset="utf-8"><title>CNC detail review v25</title><style>body{font:16px sans-serif;max-width:1400px;margin:30px;background:#f4f1eb}img{width:100%}section{background:white;margin:25px 0;padding:20px}td,th{padding:6px;border-bottom:1px solid #ccc}</style><h1>CNC structure detailing / measurement review</h1><p>36 wood records. Zero wood overlap in joint preview. 137 defined feature groups, 60 hardware-blocked groups, 4 design blockers. No production toolpaths, final fit, hardware holes or manufacturing approval.</p><p><a href="cpu-measurement.svg">CPU rail measurement drawing</a> · <a href="vpin-cnc-detail-preview.FCStd">Joint preview FCStd</a> · <a href="../review/index.html">Unmodified active baseline gallery</a></p>']
