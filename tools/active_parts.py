...
def load_path(name):
    ...
    if name.startswith('ClosedSupportDoubler'):return 'Closed landing/latch and independent prop reactions -> metal seat/clevis/backing -> combined shaped reinforcement -> cabinet side'
...
def register(doc):
    ...
    add('CapturedBottomV20','KEEP','Bottom shear panel and local equipment supports',18,'XY; inside up; front arrow','four-edge capture; standardized vent slots and filter mounts; CPU hardware blocked')
    ...
    add('ClosedSupportDoublerLeftV19 ClosedSupportDoublerRightV19','COMBINE','One shaped landing/latch/prop union per side; all three reaction zones retained',18,'YZ; inside face','combined union profile; matched hardware patterns pending')
