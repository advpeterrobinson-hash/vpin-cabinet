...
def main() -> int:
    ...
    checks = [
        ...
        ("rear aperture passes PC height", ah >= float(case["installed_height_z_mm"]) + float(pc["shelf_thickness_z_mm"]) + 40.0, f"opening H {ah:.1f}"),
        ...
        ("stowed shelf stays inside cabinet", sy0 + sd <= float(cab["side_length_mm"]) - wood + 0.1, f"rear edge {sy0+sd:.1f} mm"),
        ("service shelf exits rear", sy1 > float(cab["side_length_mm"]) - wood, f"service front Y {sy1:.1f}"),
    ...
    print(f"Installed open-case orientation {cw:.1f} x {cd:.1f} x {case['installed_height_z_mm']} mm")
